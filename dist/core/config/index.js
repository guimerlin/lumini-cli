...
function readConfigFile(filePath) {
    if (existsSync(filePath)) {
        try {
            ...
            console.warn(`Aviso: Falha ao ler ou parsear o arquivo de configuração em ${filePath}. Assumindo vazio.`);
            return {};
        }
    }
    return {};
...
export function initConfigs() {
    if (!existsSync(LOCAL_CONFIG_PATH)) {
        ...
        console.log(`Arquivo de configuração local já existe em ${LOCAL_CONFIG_PATH}`);
    }
    ensureGlobalConfig();
}
/**
...
export function getConfig(key) {
    ...
    if (key) {
        if (key in localConfig &&
            !localConfig[key].startsWith("your-")) {
            return localConfig[key];
        }
        if (key in globalConfig &&
            !globalConfig[key].startsWith("your-")) {
 ...
 */
export function setConfig(key, value, scope = "local") {
    ensureGlobalConfig();
 ...
 */
export function deleteConfig(key, scope = "local") {
    ensureGlobalConfig();
    if (scope === "global" || scope === "all") {
        const globalConfig = readConfigFile(GLOBAL_CONFIG_PATH);
        if (key in globalConfig) {
