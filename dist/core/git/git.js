 ...
 */
export function filterIgnoredFiles(files) {
    if (files.length === 0)
        return [];
    try {
 ...
 */
export function getCurrentVersion(cwd = process.cwd()) {
 ...
 */
export function createTag(version, message) {
...
export function restoreUndoSnapshot() {
    ...
    try {
        ...
        console.log("✔ Estado do repositório restaurado com sucesso! Os commits gerados foram desfeitos.");
    }
    catch {
        throw new Error("Nenhum ponto de restauração (undo) do Lumini foi encontrado para este repositório.");
    }
}
